...
func SetSend(id int64, val string) {
	userID := userSendIDToRdb(id)
	_, err := model.Bot.Rdb.Set(userID, val, time.Minute*5).Result()
	if err != nil {
		log.Println(err)
	}
}

func GetSend(id int64) string {
	userID := userSendIDToRdb(id)
	have, err := model.Bot.Rdb.Exists(userID).Result()
	if err != nil {
		log.Println(err)
...
func GetExpireTimeSend(id int64) time.Duration {
	userID := userSendIDToRdb(id)
	result, err := model.Bot.Rdb.TTL(userID).Result()
	if err != nil {
		return 0
	}
...
func userSendIDToRdb(userID int64) string {
	return "user_send:" + strconv.FormatInt(userID, 10)
}
